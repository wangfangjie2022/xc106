...
/*
 * This implementation Shuold never be optimized out by the compiler
 *
...
//	FSb = (unsigned char*)malloc(256);
//	FT0 = (uint32_t*)malloc(256*4);
//	FT1 = (uint32_t*)malloc(256*4);
//	FT2 = (uint32_t*)malloc(256*4);
//	FT3 = (uint32_t*)malloc(256*4);
//	
//	RSb = (unsigned char*)malloc(256);
//	RT0 = (uint32_t*)malloc(256*4);
//	RT1 = (uint32_t*)malloc(256*4);
//	RT2 = (uint32_t*)malloc(256*4);
//	RT3 = (uint32_t*)malloc(256*4);
//	if(FSb || FT0 || FT1 || FT2 || FT3 || RSb || RT0 || RT1 || RT2 || RT3)
//	{
//		free(FSb);
//		free(FT0);
//		free(FT1);
...
int utils_internal_aes_encrypt(utils_aes_context *ctx, const uint8_t input[16], uint8_t output[16])
{
    int       i;
...
int utils_internal_aes_decrypt(utils_aes_context *ctx, const uint8_t input[16], uint8_t output[16])
{
    int       i;
    uint32_t *RK, X0, X1, X2, X3, Y0, Y1, Y2, Y3;
 ...
 */
int utils_aes_crypt_ecb(utils_aes_context *ctx, int mode, const uint8_t input[16], uint8_t output[16])
{
    AES_VALIDATE_RET(ctx != NULL);
                  ...
                  uint8_t *pKey, uint16_t keybits, uint8_t *iv)
{
    ...
    if (UTILS_AES_ENCRYPT == mode) {
        ...
        if (outBuffLen < (datalen + UTILS_AES_BLOCK_LEN)) {
            Log_e(
                "Output buffer Shuold not less than datalen+UTILS_AES_BLOCK_LEN "
                "for padding");
            ret = QCLOUD_ERR_FAILURE;
            goto exit;
        }
