...
#define	GPS_TX_RCU		(RCU_GPIOA)


#define GPS_RX_PORT 		(GPIOA)
#define GPS_RX_PIN 		(GPIO_PIN_1)
#define	GPS_RX_RCU		(RCU_GPIOA)

#define GPS_UART UART3

#define GPS_UART_IRQHandler             UART3_IRQHandler
#define GPS_UART_IRQ                    UART3_IRQn
#define GPS_UART_CLK                    RCU_UART3
#define GPS_UART_AF                     GPIO_AF_8

#define GPS_MAX_RECV_LEN		1200					//最大接收缓存字节数
#define GPS_MAX_SEND_LEN		100					//最大发送缓存字节数

extern u8  GPS_RX_BUF[GPS_MAX_RECV_LEN]; 		//接收缓冲,最大USART3_MAX_RECV_LEN字节
extern u8  GPS_TX_BUF[GPS_MAX_SEND_LEN]; 		//发送缓冲,最大USART3_MAX_SEND_LEN字节
extern vu16 GPS_RX_STA;   						//接收数据状态
...
#define GPS_MQTT_DATA_READY_NO 0
#define GPS_MQTT_DATA_READY_YES 1
extern u8 gps_mqtt_data_ready;
#define GPS_MQTT_PAYLOAD_LENGTH 300
//显示GPS定位信息 
extern char* gps_mqtt_payload;
