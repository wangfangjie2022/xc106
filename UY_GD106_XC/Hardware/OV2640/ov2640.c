...
#define SCCB_WR_Reg ov2640_write
#define SCCB_RD_Reg ov2640_read
...
{ 
	...
	gpio_mode_set(DCI_PWD_PORT, GPIO_MODE_OUTPUT, GPIO_PUPD_PULLUP, DCI_PWD_PIN);
  gpio_output_options_set(DCI_PWD_PORT, GPIO_OTYPE_PP, GPIO_OSPEED_MAX,DCI_PWD_PIN);
	
	gpio_mode_set(DCI_RST_PORT, GPIO_MODE_OUTPUT, GPIO_PUPD_PULLUP, DCI_RST_PIN);
  gpio_output_options_set(DCI_RST_PORT, GPIO_OTYPE_PP, GPIO_OSPEED_MAX,DCI_RST_PIN);
...
//	while(1)
	...
	if(reg!=OV2640_MID)
	{
		SEGGER_RTT_printf2("MID1:%d\r\n",reg);
//		return 1;
	}
	else
	{
		SEGGER_RTT_printf2("MID2:%d\r\n",reg);
	}
	...
	if(reg!=OV2640_PID)
	{
		SEGGER_RTT_printf2("HID1:%d\r\n",reg);
//		return 2;
	}  
	else
	{
		SEGGER_RTT_printf2("HID2:%d\r\n",reg);
	}
