			...
			case NETWORK_CONNECTING:		//连接操作
				{
						...
						if(serverkeyerror==1)
						{
							...
							char ucMqttUrl[128];
							memset(ucMqttUrl,0,128*sizeof(char));
							...
							//订阅 属性回复 主题
							//dm_property_post_reply(&topic,mqttobject.productId, mqttobject.DeviceName);
							//MQTTSubscribe(&g_ATModule, topic, QOS0, PropertySubCallback,0);
							info_subVolt(&topic);//主题: order_volt/设备ID/变压器ID 服务器设置DTU下面的变压器的特性，是否打开风机，定时时间
							MQTTSubscribe(&g_ATModule, topic, QOS1,info_subVoltCallback,0);
							info_subDTU(&topic);//DTU：主题: order_device/设备ID  服务器设置DTU的特性，如是否主动上报，是否重启
							MQTTSubscribe(&g_ATModule, topic, QOS1,info_subDTUCallback,0);
							//订阅 OTA升级参数 主题
